# Petal Panic — Recoil / Hit Reaction

Recoil is **how a victim reacts physically when hit**. It is separate from damage:
damage drains a health/energy pool; recoil is the physical reaction — being shoved,
being stunned, and (for heroes) briefly becoming invulnerable. Two attacks can deal
identical damage while producing very different recoil.

The guiding idea: **recoil belongs to the thing that is hitting, not to the moment of
the hit.** Whatever deals the blow — an attack hitbox, a projectile, an explosion, or
an enemy's own body — carries its own recoil character. The engine reads that character
...
## 1. What recoil describes

A recoil setting expresses four things about a hit:
...
Recoil strength has two contributions:
...
little, while one charging straight in adds a lot. This makes recoil physical rather
...
Direction is part of the recoil character, so each attack declares how it should throw
its victim.
...
body-height or two; a light poke lifts nothing. Stronger recoil naturally produces a
...
loses control — for the recoil's duration. Physics keep running during the stun so the
...
## 7. Where recoil lives

Recoil is carried by the source of the hit:

| Source of the hit | Who carries the recoil |
|-------------------|------------------------|
...
Because recoil is data on the source, making a new attack shove harder (or not shove at
...
Because body-contact recoil includes the motion contribution, an enemy's body hit scales
...
Every way an enemy can hurt the hero, and how that hit should read in terms of recoil.
This is a design description, not tuning values.

| Enemy attack | How it lands | Recoil character on the hero |
|--------------|-------------|------------------------------|
...
Every way the hero can hurt an enemy, and how that hit should read in terms of recoil on
the enemy. v1 scope applies recoil to the three committed attacks only; the rest deal
damage without shoving (for now).

| Hero attack | Commitment | Recoil character on the enemy |
|-------------|-----------|-------------------------------|
| Normal melee | Low (quick poke) | v1: damage only, no recoil. (Future: small flinch.) |
...
| Thorn projectile | None (ranged) | No recoil — pure damage. Keeps basic fire safe and non-committal. |
...
"I fired a shot." Recoil is the tool that makes that contrast legible.
