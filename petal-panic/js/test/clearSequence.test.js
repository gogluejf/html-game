...
test('clear sequence does NOT fire for the boss checkpoint (BLOCKER 6)', () => {
  ...
  // the 4 exit routes into the BOSS ZONE (currentArea = AREA_BOSS = 5), not
  // into a nonexistent fifth area. The clear sequence itself still runs (it is
  // the normal transition per boss-arena.md §1); what must never happen is the
  ...
  // boss zone (AREA_BOSS = 5).
  U.onExitFlagReached('1-4', 5);
  const s = U.getClearSequence();
  assert.equal(s.pendingArea, 5, 'clearing 4 advances to the boss zone');
  // The boss zone (currentArea = 5) is set by the clear sequence; it is the
  // final zone, so the hero must never end up on an ordinary area past 4.
  stepClear(DT, BANNER_STEPS + FADE_OUT_STEPS);
  const hero = U.getHero();
  assert.equal(hero.currentArea, 5, 'hero is in the boss zone after 4 exit');
});
