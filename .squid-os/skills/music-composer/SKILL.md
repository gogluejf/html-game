...
```bash
# Compose: builds or REPLACES the whole state file from a tracks JSON (full overwrite)
...
- The state file is generated by compose.py only — never hand-edit it. Use `add`/`remove` for incremental playlist edits.
