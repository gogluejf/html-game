...
- **Drums** = an array of 4 sets (index = phrase): `[none, light, medium, full]`. Each set is 32 entries of `{k,s,h}` booleans. Stack instruments per level: light=snare, medium=+kick, full=+busy hats/ghosts.
- Optional **leadLayers**: `[null,null,bankA,bankB]` — extra harmony/counter line that appears on phrases 2 & 3.
