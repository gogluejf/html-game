...
Run the full loop end-to-end (execute → verify → review A/B → resolve → record → commit → mark-done → next task) **without pausing for the user between tasks**. Only stop to ask the user on: a reviewer disagreement that needs a human tie-break, a BLOCKED task after 2 fix cycles, or a wave boundary if they've asked for a checkpoint. Otherwise continuous.
