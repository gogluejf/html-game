...
## Wave 7 gate — M7 (7.1–7.4) → EPIC DONE

**Gate tests:** run-all.mjs 60/60 vs baseline 31/31 (+29 new suites/tests incl. the new effectCatalog.test.js; no new failures). effectRegression.test.js 21/21 (behavior preservation intact). effectCatalog.test.js 87/87 (new — closes DoD §4 "all 25 effects have a passing catalog test"). Pre-existing flakes (barrel.solid, contextualAim ~25%) re-run clean across 3× runs.

**Wave review (dual):**
- Review A (line-level): PASS (3 minor) — stale `render(...)` hook bullet in CATALOG JSDoc (aspirational, not wired); theater gated on Theater.active not Debug.enabled (invariant maintained by handleDebugToggle coupling, acceptable); "twenty-five … §1–§24 plus §26" reads slightly mechanical. No blockers/majors. Verified: engine core byte-for-byte unchanged by M7 (all additions after resetEffects()); debug-only gating correct; input correctness (arrows guarded, gamepad edge-triggered, Esc→debug, F1/F2 exit, gameplay frozen, screen nav suppressed); state-leak closed (open/close resetEffects + test); extensibility contract intact; deterministic tests; YAML field-accurate; effects.md stays concept-level.
- Review B (intent/doc): FAIL round 1 (1 blocker, 4 major) — (B1) 7.4 progress uncommitted + no M7 gate commit (tree not clean); (B2) effects.md documents Beam hold phase but beam.js implements flash-in+fade-out only; (B3) centralized theater catalog + POOL_DRIVEN/SHAKE_PROXY sets in index.js contradicts "no engine change"; (B4) hero.yaml beam under hero.effects overclaims hitbox origin/facing alignment; (B5) no dedicated catalog test existed (DoD §4).

**Resolution:** (B1) RESOLVED by this gate commit (progress + all M7 work now committed; tree clean). (B5) ACCEPTED (fix) — created petal-panic/js/test/effectCatalog.test.js (87 tests): every catalog type resolves through real fireManual, uniform lifecycle surface, one full frame without throwing, catalog count/order/uniqueness, representative draw calls (pool-driven bursts drawn via particles.activeItems). Auto-discovered by run-all (59→60). (B2) ACCEPTED (doc fix) — added a Deferred note that Beam v1 ships ignition+fade-out only (no hold window yet); follow-up can add the hold. (B3) ACCEPTED (doc precision) — refined the Extensibility Contract to state plainly that pool-driven/state-only effects additionally need a small data-table note in the theater (still data, not behavioral engine logic), and that the engine CORE never changes. (B4) ACCEPTED (doc fix) — rewrote the hero.yaml beam comment to state accurately that the engine passes the object OWNING the effects array as the carrier, so to mirror the attack collision area the effects must be declared on the carrier whose origin/facing match the active hitbox. Re-verify: run-all 60/60 ×3, effectCatalog 87/87 ×3, effectRegression 21/21, YAML valid, markdown coherent. A's 3 minors accepted as-is (cosmetic/aspirational, non-blocking).

**EPIC COMPLETE.** All 28 tasks done + committed (one commit per task + 7 gate commits). Full suite green vs baseline (60/60 vs 31/31). Preservation regression green (21/21). All 25 effects have passing catalog test + theater demo. Theater: KeyB shortcut, steps 25 effects in catalog order, kbd + gamepad, Esc returns to debug, F1/F2 exits. Adding an effect = one file + one registration + one theater row. Heat Distortion deferred (documented, NOT implemented). Carrier interface documented as ready for future marker/box/radius attachment. Gate commit: `effects: wave 7 gate — EPIC DONE`.
