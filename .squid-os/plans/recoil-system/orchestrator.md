# Orchestrator — recoil-system

Execution contract for running the `recoil-system` plan with **no human between tasks**.
The runner (plan-runner / main session) follows this file exactly. Modeled on the
hero-engine-doc-alignment orchestrator, adapted to the recoil system's specifics.

## Source of truth & precedence

1. `petal-panic/docs/architecture/recoil.md` — **wins over existing code** whenever they
...
2. This plan (`recoil-system.md`) — task scope, acceptance criteria, verify commands,
   testing convention.
...
4. Existing code — preserved unless a task explicitly changes it. **Critically:** the
   existing enemy→hero recoil (contact/projectile/heavyProj/explosion via
...
Every executor and reviewer prompt MUST include the `recoil.md` sections referenced by the
...
1. **Recoil math tests are pure node unit tests on `recoil.js`** (no DOM): assert exact
...
3. **Attack-recoil tests use the direct-intent pattern**: set the hero's attack active
...
```
Wave 1: 1.1 → 1.2 → 1.3     (core mechanic: applyRecoil, enemy hitstun, wiring)
Gate 1: full suite vs baseline + wave review + commit
Wave 2: 2.1 → 2.2 → 2.3     (hero attacks carry recoil; 2.3 is the feel/regression pass)
...
Ordering notes:
- 3.4 (preserve existing enemy→hero recoil) runs BEFORE 3.3 (final handoff) so the
...
pass/fail per file in `.squid-os/plans/recoil-system/baseline.txt`. Known pre-existing
...
ALSO at reset: capture the **pre-refactor enemy→hero recoil reference values** (the four
sources' resulting hero vx/vy + rec + intangible timers) into
`.squid-os/plans/recoil-system/recoil-reference.txt`. Task 3.4 asserts against this
...
7. **Record.** Append both verdicts + resolution to `.squid-os/plans/recoil-system/reviews.md`
   (one block per task).
8. **Task commit — AFTER review only.** Verify green AND both verdicts recorded AND all
   accepted findings fixed → runner commits the reviewed state: `recoil <task-id>: <task
...
1. Run the FULL existing test suite: every `petal-panic/js/test/*.test.js` (or
   ...
   - From Wave 3 onward ALSO run the preservation regression
     (`recoilRegression.test.js`) — it must stay green at every gate.
...
3. Gate commit: `recoil: wave <n> gate` (plan/progress/reviews bookkeeping only).

## Cross-cutting quality rubric (identical text for BOTH reviewers)

You are reviewing a diff against the Petal Panic recoil system. The design doc
(`recoil.md`) is the single source of truth and wins over prior code.

Check ONLY these. Report each finding as: file:line — issue — doc § reference — severity
(blocker/major/minor).

1. **Doc conformance**: does the diff implement the cited `recoil.md` behavior exactly?
...
2. **Recoil lives on the attacker, not the call site**: recoil is data carried by the
   ...
   single `applyRecoil(victim, attacker, recoil, normal)` is the only consumer.
3. **One mechanism, both directions**: hero and enemy victims use the SAME `applyRecoil`.
...
5. **Scope discipline (v1)**: recoil applies to ENEMIES only, from sweep/cartwheel/supermove.
   Normal melee, Thorn, and special projectiles carry NO recoil in v1. Flag any recoil
   leaking onto out-of-scope attacks.
6. **No duplication**: no second copy of recoil math, knockback application, or stun-TTL
   bookkeeping. New logic routes through `applyRecoil` + the unified timers.
7. **No magic numbers at call sites**: recoil values live in heroDefs.js (per-attack) /
   enemy defs (bodyRecoil) / named constants — never inline in update.js handlers.
...
```
You are implementing ONE task from the Petal Panic recoil-system plan.
...
{inline the relevant recoil.md sections, e.g. §2 strength, §4 direction, §10/§11 matrices}
...
- Recoil is DATA on the attacking object; applyRecoil is the ONLY consumer. No
...
- For task 3.4 specifically: capture/assert against the pre-refactor reference values in
  recoil-reference.txt; do not alter the existing hero→(enemy) or enemy→(hero) feel.
...
Focus: items 2, 4, 5, 7, 9 (recoil-on-attacker, behavior preservation, v1 scope, magic
...
duplication introduced by the wave, drift from the doc's recoil rules (§2, §4, §8,
...
3. Preservation regression (`recoilRegression.test.js`) green — existing enemy→hero recoil
   provably unchanged.
